...
13. When a role deploys files to the host, ALWAYS add them to `playbooks/cleanup.yml` only (unified cleanup; `molecule/default/cleanup.yml` imports it — one list to maintain).
...
- **LXC containers on WAN hosts** (ordinary Debian services): container NAT bridge `vmbr_ct` from `proxmox_bridges` — NOT `proxmox_all_bridges[1]` unless the role explicitly needs LAN topology; OpenWrt mesh/bridge LXC are exceptions (WAN/backhaul bridges per role)
...
it to `playbooks/cleanup.yml` (unified cleanup). `molecule/default/cleanup.yml`
is a one-line import of that playbook — do not duplicate removal lists.
...
VMIDs: see `group_vars/all.yml` and `playbooks/cleanup.yml`. When adding a new
service, add its VMID to `playbooks/cleanup.yml` and `tasks/cleanup_lan_host.yml`.
...
`monitoring_nodes`), each container needs a unique IP on the shared LAN
subnet. Use the host's index in its flavor group:
`offset + groups['<flavor>'].index(inventory_hostname)` together with
`tasks/lxc_lan_network.yml` (LAN hosts).

**WAN hosts:** container addressing uses `tasks/lxc_wan_or_lan_network.yml`:
per-host NAT bridge (`10.99.{container_subnet_id}.{offset}`) — no household
subnet, no collision with host management IPs. Offsets are unique per service
...
- Spacing: indexed offset blocks MUST NOT overlap (see project-structure
  rules — kiosk / rsyslog / netdata spacing).
