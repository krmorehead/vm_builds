...
3. `deploy_stamp` records what ran. Rollback tasks SHOULD check `ansible_local.vm_builds.plays` to determine what needs undoing. NEVER attempt to roll back a feature that was never applied.
...
│   ├── Reverts: UCI config, installed packages, firewall rules, cron jobs
│   ├── Preserves: VM, base OS, all other features
│   ├── Trigger: cleanup.sh <feature>-rollback [env-file]
│   │            → build.py --playbook cleanup --tags <feature>-rollback
│   └── Example: remove banIP, revert SSH config, restart dropbear
│
